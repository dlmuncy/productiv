...
import { CheckCircle2, Circle, Bot, Plus } from 'lucide-react';
...
export function MindMapView({ tasks, agents, projectColor, projectName, onToggleComplete }: MindMapViewProps) {
  ...
  const { nodes, edges } = useMemo(() => {
    ...
    // Place agent nodes in a circle around root
    ...
    // Unassigned tasks branch directly from root
    ...
    >
      ...
      {nodes.length === 0 && (
        <div className="absolute inset-0 flex items-center justify-center">
          <p className="text-slate-400">Add tasks to see the mind map</p>
        </div>
      )}
      ...
      >
          ...
          {edges.map((edge, i) => {
            ...
            return (
              <line
                ...
                x2={to.x} y2={to.y}
                stroke={from.color}
                strokeWidth={from.type === 'root' ? 2 : 1.5}
                strokeOpacity={0.3}
                strokeDasharray={to.type === 'task' ? '4 2' : '0'}
              />
            );
          })}
        </svg>
        ...
        {nodes.map((node) => {
          ...
          const size = isRoot ? 80 : isAgent ? 56 : 40;
          return (
            <div
              key={node.id}
              className="absolute flex items-center justify-center rounded-full font-medium text-center transition-all hover:scale-110 cursor-pointer"
              style={{
                width: size,
                height: size,
                left: node.x - size / 2,
                top: node.y - size / 2,
                backgroundColor: node.completed ? '#e2e8f0' : node.color,
                color: 'white',
                fontSize: isRoot ? '13px' : isAgent ? '11px' : '9px',
                boxShadow: isRoot ? '0 4px 20px rgba(0,0,0,0.15)' : '0 2px 8px rgba(0,0,0,0.1)',
                border: node.completed ? '2px solid #94a3b8' : '2px solid white',
                lineHeight: 1.1,
                padding: '4px',
            ...
            >
              ...
              {isAgent && (
                <div className="flex flex-col items-center gap-1">
                  <Bot className={isRoot ? 'w-5 h-5' : 'w-4 h-4'} />
                  <span className="line-clamp-1 px-1">{node.label}</span>
                </div>
              )}
              {node.type === 'task' && (
