    ...
    <div className="flex-1 overflow-y-auto scrollbar-thin mesh-bg p-6">
      <div className="max-w-6xl mx-auto space-y-6">
        ...
        <div className="card p-8 animate-slide-up relative overflow-hidden">
          <div
            className="absolute top-0 right-0 w-64 h-64 rounded-full opacity-10 -mr-20 -mt-20"
            style={{ backgroundColor: projectColor }}
          />
          <div className="relative">
            <div className="flex items-center gap-2 mb-2">
              <Target className="w-5 h-5" style={{ color: projectColor }} />
              <h2 className="text-sm font-semibold text-slate-500 uppercase tracking-wider">{projectName} Overview</h2>
            </div>
            <div className="flex items-end gap-8">
              <div>
                <p className="text-5xl font-bold text-slate-900">{stats.completionRate}%</p>
                <p className="text-sm text-slate-500 mt-1">Project Completion</p>
              </div>
            ...
            <div className="mt-4 h-3 bg-slate-100 rounded-full overflow-hidden">
              <div
                className="h-full rounded-full transition-all duration-1000 flex"
                style={{ width: `${stats.completionRate}%` }}
              >
                <div className="flex-1" style={{ backgroundColor: projectColor }} />
              </div>
            </div>
          </div>
        </div>
        ...
        <div className="grid md:grid-cols-2 gap-6">
          ...
          <div className="card p-6 animate-slide-up" style={{ animationDelay: '0.1s' }}>
            <h3 className="font-bold text-slate-900 mb-4">Task Status Distribution</h3>
          ...
          <div className="card p-6 animate-slide-up" style={{ animationDelay: '0.2s' }}>
            <h3 className="font-bold text-slate-900 mb-4">Open Tasks by Priority</h3>
            <div className="space-y-3">
              {[
                { label: 'Urgent', value: stats.byPriority.urgent, color: '#f43f5e' },
                { label: 'High', value: stats.byPriority.high, color: '#f59e0b' },
                  ...
                  <div className="h-6 bg-slate-100 rounded-md overflow-hidden">
                    <div
                      ...
                      style={{
                        ...
                        minWidth: p.value > 0 ? '24px' : '0',
                      }}
                    >
                      {p.value > 0 && <span className="text-[10px] text-white font-bold">{p.value}</span>}
                    </div>
                  </div>
                </div>
              ))}
        ...
        {stats.byAgent.length > 0 && (
          <div className="card p-6 animate-slide-up" style={{ animationDelay: '0.3s' }}>
            ...
            <div className="grid sm:grid-cols-2 lg:grid-cols-4 gap-4">
              {stats.byAgent.map(({ agent, total, completed }) => (
                <div key={agent.id} className="p-4 rounded-lg border border-slate-200 bg-slate-50/50">
        ...
        {/* Velocity trend (simulated from task data) */}
        <div className="card p-6 animate-slide-up" style={{ animationDelay: '0.4s' }}>
          ...
          <div className="flex items-end gap-2 h-32">
            {(() => {
              const recentTasks = [...tasks].sort((a, b) => new Date(b.created_at).getTime() - new Date(a.created_at).getTime());
              const bars = [];
              for (let i = 6; i >= 0; i--) {
                const date = new Date();
                ...
                bars.push({ day: date.toLocaleDateString('en-US', { weekday: 'short' }), total: dayTasks.length, completed });
              }
              const maxBar = Math.max(...bars.map((b) => b.total), 1);
              return bars.map((b, i) => (
                <div key={i} className="flex-1 flex flex-col items-center gap-1">
                  <div className="w-full flex-1 flex items-end">
                    <div className="w-full rounded-t-md transition-all duration-700 relative" style={{ height: `${(b.total / maxBar) * 100}%`, backgroundColor: projectColor, minHeight: b.total > 0 ? '8px' : '2px' }}>
                      {b.completed > 0 && (
                        <div className="absolute bottom-0 w-full rounded-t-md bg-emerald-400" style={{ height: `${(b.completed / b.total) * 100}%` }} />
                      )}
                    </div>
                  </div>
          ...
          <div className="flex items-center gap-4 mt-3 text-xs text-slate-500">
            <span className="flex items-center gap-1.5"><span className="w-2.5 h-2.5 rounded" style={{ backgroundColor: projectColor }} /> Created</span>
            <span className="flex items-center gap-1.5"><span className="w-2.5 h-2.5 rounded bg-emerald-400" /> Completed</span>
          </div>
        </div>
      </div>
    </div>
