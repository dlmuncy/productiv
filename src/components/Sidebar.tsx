...
export function Sidebar({ projects, activeView, activeProjectId, onNavigate, onNewProject }: SidebarProps) {
  ...
  const navItems = [
    ...
    { id: 'connections' as View, label: 'Integrations', icon: Plug },
      ...
      <div className="px-3 py-3">
        <div className="relative">
          ...
          <input
            type="text"
            placeholder="Search..."
            className="w-full pl-8 pr-12 py-2 text-sm bg-slate-100 rounded-lg border border-transparent focus:outline-none focus:bg-white focus:border-slate-200 transition-all"
          />
          <kbd className="absolute right-2.5 top-1/2 -translate-y-1/2 text-[10px] text-slate-400 bg-white border border-slate-200 rounded px-1 py-0.5 flex items-center gap-0.5">
            ...
            {projects.map((p) => (
              <button
                ...
                className={cn(
                  ...
                  activeView === 'project' && activeProjectId === p.id && 'sidebar-item-active'
                )}
              >
                <span
                ...
                {p.agent_managed && (
                  <Bot className="w-3 h-3 text-brand-500 flex-shrink-0" />
                )}
              </button>
            ))}
          </div>
...
export function TopBar({ title, subtitle, actions }: TopBarProps) {
  return (
    <header className="h-16 flex items-center justify-between px-6 border-b border-slate-200/80 bg-white/80 backdrop-blur-xl flex-shrink-0">
      <div>
